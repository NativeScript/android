...
public final class MultiDex
{
	...
	{
		// quick and dirty hack for Android 5 until dex files are located in apk
		// root
		return false;
		/*
		 * boolean isMultidexCapable = false; if (versionString != null) {
		 * Matcher matcher =
		 * Pattern.compile("(\\d+)\\.(\\d+)(\\.\\d+)?").matcher(versionString);
		 * if (matcher.matches()) { try { int major =
		 * Integer.parseInt(matcher.group(1)); int minor =
		 * Integer.parseInt(matcher.group(2)); isMultidexCapable = (major >
		 * VM_WITH_MULTIDEX_VERSION_MAJOR) || ((major ==
		 * VM_WITH_MULTIDEX_VERSION_MAJOR) && (minor >=
		 * VM_WITH_MULTIDEX_VERSION_MINOR)); } catch (NumberFormatException e) {
		 * // let isMultidexCapable be false } } } Log.i(TAG, "VM with version "
		 * + versionString + (isMultidexCapable ? " has multidex support" :
		 * " does not have multidex support")); return isMultidexCapable;
		 */
	}

	public static void installSecondaryDexes(ClassLoader loader, File dexDir, List<File> files) throws IllegalArgumentException, IllegalAccessException, NoSuchFieldException, InvocationTargetException, NoSuchMethodException, IOException
	{
		if (!files.isEmpty())
		{
