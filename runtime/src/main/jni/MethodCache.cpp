...
void MethodCache::Init() {
    ...
    RESOLVE_METHOD_OVERLOAD_METHOD_ID = env.GetMethodID(RUNTIME_CLASS, "resolveMethodOverload",
                                                        "(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;");
    assert(RESOLVE_METHOD_OVERLOAD_METHOD_ID != nullptr);

    RESOLVE_CONSTRUCTOR_SIGNATURE_ID = env.GetMethodID(RUNTIME_CLASS, "resolveConstructorSignature",
                                                       "(Ljava/lang/Class;[Ljava/lang/Object;)Ljava/lang/String;");
    assert(RESOLVE_CONSTRUCTOR_SIGNATURE_ID != nullptr);
}

MethodCache::CacheMethodInfo MethodCache::ResolveMethodSignature(const string &className,
...
MethodCache::CacheMethodInfo MethodCache::ResolveConstructorSignature(const ArgsWrapper &argWrapper,
...
string MethodCache::EncodeSignature(const string &className, const string &methodName,
                                    const FunctionCallbackInfo<Value> &args, bool isStatic) {
...
string MethodCache::GetType(Isolate *isolate, const v8::Local<v8::Value> &value) {
    ...
    if (value->IsObject()) {
        ...
        auto maybeNullNode = objVal->GetPrivate(isolate->GetCurrentContext(), Private::New(isolate,
                                                                                           V8StringConstants::GetNullNodeName()));

        if (!maybeNullNode.IsEmpty()) {
            Local<Value> nullNode;
            maybeNullNode.FromMaybe(nullNode);
            ...
            if (!nullNode.IsEmpty()) {
                ...
                DEBUG_WRITE("Parameter of type %s with NULL value is passed to the method.",
                            type.c_str());
                return type;
            }
        }
    ...
    if (value->IsArray() || value->IsArrayBuffer() || value->IsArrayBufferView() ||
        value->IsTypedArray()
    ...
    else if (value->IsBoolean() || value->IsBooleanObject() || value->IsFalse() ||
             value->IsTrue()) {
        type = "bool";
    }
...
string MethodCache::ResolveJavaMethod(const FunctionCallbackInfo<Value> &args,
    ...
    jstring signature = (jstring) env.CallObjectMethod(runtime->GetJavaRuntime(),
                                                       RESOLVE_METHOD_OVERLOAD_METHOD_ID,
                                                       (jstring) jsClassName,
                                                       (jstring) jsMethodName, arrArgs);
...
string MethodCache::ResolveConstructor(const FunctionCallbackInfo<Value> &args, jclass javaClass,
                                       bool isInterface, Local<Object> outerThis) {
    JEnv env;
    string resolvedSignature;
    ...
    if (argConverter.IsValid()) {
        ...
        jstring signature = (jstring) env.CallObjectMethod(runtime->GetJavaRuntime(),
                                                           RESOLVE_CONSTRUCTOR_SIGNATURE_ID,
                                                           javaClass, javaArgs);
