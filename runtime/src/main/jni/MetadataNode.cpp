...
Local<Object> MetadataNode::CreateExtendedJSWrapper(Isolate *isolate, ObjectManager *objectManager, const string& proxyClassName)
{
	...
	if (cacheData.node != nullptr)
	{
		...
		extInstance->SetPrototype(extdCtorFunc->Get(ArgConverter::ConvertToV8String("prototype")));
...
Local<Object> MetadataNode::CreateJSWrapper(Isolate *isolate, ObjectManager *objectManager)
{
	...
	else
	{
		...
		if (!obj.IsEmpty())
		{
			...
			obj->Set(ArgConverter::ConvertToV8String("constructor"), ctorFunc);
			obj->SetPrototype(ctorFunc->Get(ArgConverter::ConvertToV8String("prototype")));
...
Local<Object> MetadataNode::CreateArrayWrapper(Isolate *isolate)
{
	...
	arr->SetPrototype(objPrototype->Get(ArgConverter::ConvertToV8String("prototype")));
	arr->SetAccessor(ArgConverter::ConvertToV8String("length"), ArrayLengthGetterCallack, nullptr, Local<Value>(), AccessControl::ALL_CAN_READ, PropertyAttribute::DontDelete);
...
Local<Object> MetadataNode::CreatePackageObject(Isolate *isolate)
{
	...
	if (ptrChildren != nullptr) {
		...
		for (auto childNode: children) {
            packageObj->SetAccessor(ctx, ArgConverter::ConvertToV8String(childNode->name),
...
{
	auto classFieldName = ArgConverter::ConvertToV8String("class");
...
void MetadataNode::SuperAccessorGetterCallback(Local<String> property, const PropertyCallbackInfo<Value>& info)
{
	try
	{
		auto thiz = info.This();
		auto isolate = info.GetIsolate();
		auto key = ArgConverter::ConvertToV8String("supervalue");
...
void MetadataNode::SetInstanceMembersFromStaticMetadata(Isolate *isolate, Local<FunctionTemplate>& ctorFuncTemplate, Local<ObjectTemplate>& prototypeTemplate, vector<MethodCallbackData*>& instanceMethodsCallbackData, const vector<MethodCallbackData*>& baseInstanceMethodsCallbackData, MetadataTreeNode *treeNode)
{
	...
	for (auto i = 0; i < instanceMethodCout; i++)
	{
		...
		if (entry.name != lastMethodName)
		{
			...
			auto funcName = ArgConverter::ConvertToV8String(entry.name);
	...
	for (auto i = 0; i < instanceFieldCout; i++)
	{
		...
		auto fieldName = ArgConverter::ConvertToV8String(entry.name);
...
void MetadataNode::SetInnerTypes(Isolate *isolate, Local<Function>& ctorFunction, MetadataTreeNode *treeNode)
{
	if (treeNode->children != nullptr)
	{
		...
		auto prototypeTemplate2 = ctorFunction->Get(ArgConverter::ConvertToV8String("prototype")).As<Object>();
		...
		for (auto curChild : children)
		{
			...
			auto innerTypeName = ArgConverter::ConvertToV8String(curChild->name);
...
void MetadataNode::ExtendedClassConstructorCallback(const v8::FunctionCallbackInfo<v8::Value>& info)
{
	try
	{
		...
		thiz->SetHiddenValue(ArgConverter::ConvertToV8String("t::implObj"), implementationObject);
...
Local<Object> MetadataNode::GetImplementationObject(const Local<Object>& object)
{
	...
	implementationObject = object->GetHiddenValue(ArgConverter::ConvertToV8String("t::implObj")).As<Object>();
...
void MetadataNode::ExtendMethodCallback(const v8::FunctionCallbackInfo<v8::Value> &info)
{
	try
	{
		...
		auto prototypeName = ArgConverter::ConvertToV8String("prototype");
		implementationObject->SetPrototype(baseClassCtorFunc->Get(prototypeName));
		implementationObject->SetAccessor(ArgConverter::ConvertToV8String("super"), SuperAccessorGetterCallback, nullptr, implementationObject);
...
void MetadataNode::CreateTopLevelNamespaces(Isolate *isolate, const Local<Object>& global)
{
	...
	for (auto treeNode : children)
	{
		...
		if (nodeType == MetadataTreeNode::PACKAGE)
		{
			...
			global->Set(ArgConverter::ConvertToV8String(node->m_treeNode->name), packageObj);
		}
	}
}
...
Local<Function> MetadataNode::Wrap(Isolate* isolate, const Local<Function>& f, const string& name, const string& origin,  bool isCtorFunc)
{
	...
	auto str = ss.str();
	auto source = ArgConverter::ConvertToV8String(str);
	auto context = isolate->GetCurrentContext();

	TryCatch tc;

	Local<Script> script;
	ScriptOrigin jsOrigin(ArgConverter::ConvertToV8String(origin));
	...
	if (!script.IsEmpty())
	{
		...
		if (!result.IsEmpty())
		{
			...
			ret->Set(ArgConverter::ConvertToV8String("__func"), f);

			auto prototypePropName = ArgConverter::ConvertToV8String("prototype");
