...
static void ResponseReceivedCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    if ((!argsObj->Has(context, ArgConverter::ConvertToV8String(isolate, "requestId")).FromMaybe(false) ||
        ...
        throw NativeScriptException("Not all parameters are present in the object argument in the call to ResponseReceived! Required params: 'requestId', `timestamp`, `type`, `response`");
    }
...
static void DataForRequestId(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    if (!argsObj->Has(context, ArgConverter::ConvertToV8String(isolate, "requestId")).FromMaybe(false) ||
        ...
        throw NativeScriptException("Not all parameters are present in the object argument in the call to DataForRequestId! Required params: 'requestId', `data`, `hasTextContent`");
    }
    ...
    if (it == responses.end()) {
        throw NativeScriptException("Response not found for requestId = " + ArgConverter::ConvertToString(requestId));
    } else {
...
static void LoadingFinishedCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    if (!argsObj->Has(context, ArgConverter::ConvertToV8String(isolate, "requestId")).FromMaybe(false) ||
            !argsObj->Has(context, ArgConverter::ConvertToV8String(isolate, "timeStamp")).FromMaybe(false)) {
        throw NativeScriptException("Not all parameters are present in the object argument in the call to LoadingFinished! Required params: 'requestId', `timeStamp`");
    }
