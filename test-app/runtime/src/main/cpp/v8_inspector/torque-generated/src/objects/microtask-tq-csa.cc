...
TNode<Microtask> DownCastForTorqueClass_Microtask_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_o, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block0.is_used()) {
    ...
    if (((CodeStubAssembler(state_).ConstexprInt31Equal(static_cast<InstanceType>(71), static_cast<InstanceType>(75))))) {
      ca_.Goto(&block3);
    } else {
      ca_.Goto(&block4);
    }
  ...
  if (block4.is_used()) {
    ca_.Bind(&block4);
    tmp8 = FromConstexpr_int32_constexpr_int32_0(state_, (CodeStubAssembler(state_).ConstexprUint32Sub(static_cast<InstanceType>(75), static_cast<InstanceType>(71))));
