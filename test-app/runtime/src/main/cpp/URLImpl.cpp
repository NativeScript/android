...
// Install URL.createObjectURL / URL.revokeObjectURL / URL.InternalAccessor +
// URL.prototype.searchParams accessor onto the realm's URL constructor.
//
// Blob IDs use `java.util.UUID.randomUUID().toString()` (lower-case
// RFC-4122 v4), producing `blob:nativescript/<uuid>` keys.
void URLImpl::InstallBlobMethods(v8::Local<v8::Context> context) {
    ...
    auto compiled = v8::Script::Compile(
        context, ArgConverter::ConvertToV8String(isolate, blob_methods))
                        .ToLocal(&script);
