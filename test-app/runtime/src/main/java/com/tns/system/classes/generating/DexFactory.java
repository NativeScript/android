...
public class DexFactory {
    ...
    private final RuntimeBindingGenerator runtimeBindingGenerator;

    private ProxyGenerator proxyGenerator;
    private HashMap<String, Class<?>> injectedDexClasses = new HashMap<>();

    public DexFactory(Logger logger, ClassLoader classLoader, File dexBaseDir, String dexThumb, ClassStorageService classStorageService, RuntimeBindingGenerator runtimeBindingGenerator) {
        this.logger = logger;
...
//        this.updateDexThumbAndPurgeCache();
        ...
        this.runtimeBindingGenerator = runtimeBindingGenerator;
    }
    ...
    public Class<?> resolveClass(String baseClassName, String name, String className, String[] methodOverrides, String[] implementedInterfaces, boolean isInterface) throws ClassNotFoundException, IOException {
        ...
        try {
            ...
            Class<?> pregeneratedClass = classStorageService.retrieveClass(fullClassName.replace("-", "_"));
...
//        }
        ...
        try {
            Class<?> binding = runtimeBindingGenerator.generateBinding(classToProxy, Arrays.asList(implementedInterfaces), Arrays.asList(methodOverrides));

            if (isInterface) {
                classStorageService.storeClass(fullClassName, binding);
            } else {
                classStorageService.storeClass(desiredDexClassName, binding);
            }
...
//    private File getDexFile(String className) {
